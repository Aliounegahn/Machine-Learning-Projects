...
train = df[df.recipients.notnull()].copy().sort_values(['id']).reset_index(drop=True)
test = df[df.recipients.isnull()].copy().sort_values(['id']).reset_index(drop=True)
...
for sender in df.sender.value_counts().index.tolist():
...
# split recipients into multiple rows
train = split_column(train.copy(), 'recipients', replace=False)

# Init tfidf vectorizers and SVD objects
...
all_mails = pd.concat([train.content, test.content])
words = word_vec.fit_transform(all_mails)
chars = char_vec.fit_transform(all_mails)
chars = svd.fit_transform(chars)

# join tfidf with numerical features
to_drop = ['content', 'datetime', 'id', 'recipients', 'sender', 'split_recipients']
xtrain = np.concatenate((words[:len(train)].toarray(), chars[:len(train)],
                         train.drop(to_drop, axis=1).values), axis=1)

xtest = np.concatenate((words[-len(test):].toarray(), chars[-len(test):],
                       test.drop(to_drop[:-1], axis=1).values), axis=1)

# get ytrain
ytrain = train['split_recipients']

# print time needed
print('took %s min %s sec' % (int((time() - start)/60),
                              round((time() - start) - int((time() - start)/60)*60, 1)))

# display fitting advancement
start = time()
print('Fitting our model...')

# train a Light GBM classifier (note that we fit a mono-label clf)
clf = LGBMClassifier(n_estimator=1000, learning_rate=0.04, max_bin=128,
                     random_state=10)
...
# get test senders
test_senders = test.sender.tolist()

# write to zero the probability that the sender is predicted
for row, sender in zip(proba, test_senders):
    if len(sender_dict[sender]) > 0:
        for label in sender_dict[sender]:
            row[label] = 0

# keep the 10 strongest labels (with highest probabilities)
pred = [sorted(range(len(elem)), key=lambda i: elem[i],
               reverse=True)[:10] for elem in proba]

# reverse mails encoding and get id
import warnings
warnings.filterwarnings('ignore')
recipients = pd.Series([' '.join(map(le.inverse_transform, elem)) for elem in pred])
recipients_id = test_info_sid['id'].reset_index(drop=True)

# merge in a dataframe
submission = pd.DataFrame({'id': recipients_id, 'recipients': recipients})

# save submission in a csv
submission.to_csv('submissions/lgbm_27labels.csv', index=False)

# print time needed
print('took %s sec' % round(time() - start, 1))
