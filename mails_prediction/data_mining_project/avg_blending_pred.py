...
# init our classifiers, we are going to average 5 lgbm, 5 random forest, and 5
# extratrees with different random state. After that we will blend them with
# different weights.
lgbm_s = [
...
# fit, predict, average and store proba predicted by each classifier
probas = []
...
# blend probas and get blend pred
proba_blend = 0
...
# display fitting advancement
start = time()
print('Save model and submission...')

# reverse mails encoding and get id
import warnings
warnings.filterwarnings('ignore')
recipients = pd.Series([' '.join(map(le.inverse_transform, elem)) for elem in pred_blend])
...
submission.to_csv('submissions/avg_blend_27labels.csv', index=False)
