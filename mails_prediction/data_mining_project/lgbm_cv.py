...
clf = LGBMClassifier(n_estimators=100, learning_rate=0.04, max_bin=128, 
...
for train_ix, test_ix in skf.split(X=train, y=train['split_recipients']):
    ...
    start = time()
    print('Performing fold %s...' % str(len(scores) + 1))
    ...
    pred = [sorted(range(len(row)), key=lambda i: row[i],
                   reverse=True)[:10] for row in proba]
    
    # compute and append map10 score
    scores.append(mapk(ytest, pred, k=10))
    
    # print time needed to perform the fold
    print('took %s sec' % round(time() - start, 1))

# print CV results
print('\nCV score mapk = %s' % round(np.mean(scores), 4))
print('          std = %s' % round(np.std(scores), 4))
